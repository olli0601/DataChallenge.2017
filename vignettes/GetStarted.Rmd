...
the functions tab
above](https://github.com/olli0601/DataChallenge.2017/docs/reference/index.html).
...
you what proportion of the data are missing for each column of the `cap` data
set. 
```{r, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
require(DataChallenge.2017)
data(cap)
df <- melt(cap, id.vars=c('ISO','YEAR'), stringsAsFactors=FALSE)
df[, {
		z <- length(value)
		z2 <-length(which(is.na(value)))
		list(N=z, N_NA=z2, P_NA= z/z2)
	 }, by=c('variable')]		
```
